...
Completed `gg` reviews exit `0` even with findings. Incomplete execution exits `1`; invalid usage exits `2`; check exit `4` means not applicable. `gg guard pre-push` retains blocking exit `1` for findings or unavailable protection, limited to secrets and large files.
