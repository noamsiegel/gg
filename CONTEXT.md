...
ADR-011 — anti-slop as a vendored, config-free JS/TS check: `checks/anti-slop.sh` runs the generic [dmmulroy/anti-slop](https://github.com/dmmulroy/anti-slop) Oxlint rules over changed JS/TS files. It is admitted under ADR-008/ADR-009: the rule set is fixed in `checks/anti-slop/gg-oxlint.config.ts`, not extensible from the repository, and the invocation (`--config <gg's file> --disable-nested-config --allow all` with the unicorn/oxc/typescript builtin plugins disabled) makes the repository's own Oxlint config unable to enable, silence, or reshape any finding. This preserves the repo-independent verdict: measured, a repository `.oxlintrc.json` that set `anti-slop/no-object-parameters` to `off` and enabled `no-console` changed nothing gg reported. It is not the non-duplication violation ADR-010 warns about, because gg reads no repository Oxlint config and repositories are not expected to run these custom rules in CI.

Two consequences distinguish it from the Python checks. First, Oxlint ships with no dependencies and the anti-slop rule modules import `@oxlint/plugins` at load time, so that one package is installed lazily under `checks/anti-slop/plugin/node_modules` on the first JS/TS review. That footprint lives inside gg's own checkout, never the repository under review, so invariant 1 holds; the install is best-effort and the check skips with exit 2 when `npm` is absent or offline. Second, the Effect rule group is deliberately not vendored: it encodes Effect architecture policy and would misfire in non-Effect repositories, failing the ADR-008 test. Oxlint and `@oxlint/plugins` are pinned to one version in two places that must move together — `ANTI_SLOP_OXLINT_VERSION` in the check and the dependency in `plugin/package.json`.
