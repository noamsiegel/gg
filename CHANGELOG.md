...
- Isolate review checks and temporary snapshot repositories from Git hook routing variables. Preserve the caller's selected index when reading staged content without allowing temporary Git writes to alter the caller's index or references.

## [3.0.2]

### Fixed

- Include explicitly selected, non-ignored untracked files in path reviews, including pathspecs resolved from nested invocation directories.
- Keep Vulture's whole-repository call graph without passing the repository file list through `argv`, preserve unknown invalid-input failures, exclude cookiecutter-owned render paths, analyze Python around declared cell magics, and run every JavaScript review mode against each file's enclosing declared workspace entry graph.

## [3.0.1]

### Fixed

- Share local objects in temporary history scans instead of repacking partial clones.
- Preserve promisor remote configuration so scans can fetch missing objects required by the selected history.
